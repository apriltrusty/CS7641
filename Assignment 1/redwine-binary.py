...
def run_decision_tree_experiments(X_train, y_train, do_gridsearch=False):
    ...
    generate_learning_curve(clf=DecisionTreeClassifier(ccp_alpha=0.004, criterion='gini'), 
                            X_train=X_train, y_train=y_train, clf_name=clf_name, 
                            clf_params=f'\nalpha=0.004, split on Gini index', score_label='F1 Score')
...
def run_boosted_tree_experiment(X_train, y_train, do_gridsearch=False):
    ...
    generate_learning_curve(clf=AdaBoostClassifier(base_estimator=DecisionTreeClassifier(max_depth=1, criterion='entropy'), 
                            n_estimators=1500, learning_rate=0.01), X_train=X_train, y_train=y_train, clf_name=clf_name, 
                            clf_params='\n1500 estimators, 0.01 learning rate', score_label='F1 Score')
...
def run_ann_experiment(X_train, y_train, do_gridsearch=False):
    ...
    generate_learning_curve(clf=MLPClassifier(hidden_layer_sizes=(300), solver='adam', learning_rate_init=0.003, max_iter=5000), 
                            X_train=X_train, y_train=y_train, clf_name=clf_name, 
                            clf_params='\n300 nodes in hidden layer, 0.003 initial learning rate', score_label='F1 Score')
    ...
    generate_complexity_curve(clf=MLPClassifier(solver='adam', learning_rate_init=0.003, max_iter=5000, random_state=0), clf_name=clf_name, 
