...
def plot_annealing_comparison():
    ...
    _, _, sa_curve_high_fast = mlrose_hiive.simulated_annealing(problem, schedule=mlrose_hiive.GeomDecay(init_temp=1e4, decay=0.99), max_attempts=200, random_state=1, curve=True)
    ...
    _, _, sa_curve_high_slow = mlrose_hiive.simulated_annealing(problem, schedule=mlrose_hiive.GeomDecay(init_temp=1e4, decay=0.999), max_attempts=200, random_state=1, curve=True)
    ...
    plt.plot(range(len(sa_curve_high_fast)), sa_curve_high_fast, label='High temperature, fast decay', color='mediumturquoise')
    plt.plot(range(len(sa_curve_low_fast)), sa_curve_low_fast, label='Low temperature, fast decay', color='lightcoral')
    plt.plot(range(len(sa_curve_high_slow)), sa_curve_high_slow, label='High temperature, slow decay', color='violet')
    plt.plot(range(len(sa_curve_low_slow)), sa_curve_low_slow, label='Low temperature, slow decay', color='slateblue')
