...
class JacksCarRental(BaseEnv):
    ...
    **termination:** 100 timesteps elapse.
    ...
    def __init__(self, size=21, max_move=5,
                 lot1_requests_distr=3, lot1_dropoffs_distr=3,
                 lot2_requests_distr=4, lot2_dropoffs_distr=2):
        self._size = size
        self._max_move = max_move

        # Poission distributions for requests and dropoffs at both lots
        self._lot1_requests_distr = TruncatedPoisson(lot1_requests_distr)
        self._lot1_dropoffs_distr = TruncatedPoisson(lot1_dropoffs_distr)
        self._lot2_requests_distr = TruncatedPoisson(lot2_requests_distr)
        self._lot2_dropoffs_distr = TruncatedPoisson(lot2_dropoffs_distr)

        # Precompute the factored transition and reward functions for both lots
        self.P1, self.R1 = open_to_close(self._lot1_requests_distr, self._lot1_dropoffs_distr, max_move, size)
        self.P2, self.R2 = open_to_close(self._lot2_requests_distr, self._lot2_dropoffs_distr, max_move, size)
        ...
        super().__init__(starts={(10, 10)}, n_actions=(max_move*2 + 1), reachable_states=states)
    ...
    def step(self, action):
        ...
        state = self._state
        action = decode_action(action, self._max_move)

        next_state = move_cars(state, action)

        requests, dropoffs = self._sample_random_elements()
        for i in range(len(next_state)):
            next_state[i] = handle_requests_and_dropoffs(next_state[i], requests[i], dropoffs[i], self._size)
...
def handle_requests_and_dropoffs(cars, requests, dropoffs, size):
    ...
    return clip(cars + dropoffs - satisfied_requests, 0, size-1)


def open_to_close(requests_distr, dropoffs_distr, max_move, size):
    ...
    P = np.zeros((size+max_move, size), dtype=np.float32)
    R = np.zeros(size+max_move)
    ...
    for requests, request_prob in requests_distr:
        ...
        for dropoffs, dropoff_prob in dropoffs_distr:
            for n in range(size+max_move):
                new_n = handle_requests_and_dropoffs(n, requests, dropoffs, size)
